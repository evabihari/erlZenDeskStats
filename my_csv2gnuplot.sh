...
if [ "$FORMAT" = "pdf" ]
then
   TERM="set terminal postscript  eps enhanced color font 'Helvetica,12'"
...
echo "  set xdata time" >> "$GNUPLOT"
echo "  set format x \"%Y/%m\" " >> "$GNUPLOT"
echo "  set xrange [\"2014/01\":\"2015/09\"]" >> "$GNUPLOT"   
echo "  set yrange [0 :*]" >> "$GNUPLOT"
echo "  set y2tics 20 nomirror tc lt 2" >> "$GNUPLOT"
echo "  set ylabel \"$YLABEL\" tc lt 1" >> "$GNUPLOT"
echo "  set y2label 'no of reported hours' tc lt 3" >> "$GNUPLOT"
...
while read filename ; do
    ...
    if [ $i == 1 ]
    then
	echo "plot \"$filename\" using $1:$2 title \"$title\" noenhanced axis $AXIS with lines " > $TMP
    else	   
 	echo "replot \"$filename\" using $1:$2 title \"$title\" noenhanced axis $AXIS with lines " >> $TMP
    fi
    echo >> "$TMP"
done
...
echo "replot \"timereports.gnuplot\" using 1:2 title \"reported hours\" noenhanced axis x1y2 with lines" >> $GNUPLOT
